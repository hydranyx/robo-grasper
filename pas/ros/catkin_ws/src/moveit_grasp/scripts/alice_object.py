...
class AliceObject(object):
    ...
    def GetObjectInformation(self):
        try:
            image_msgs = rospy.wait_for_message("/camera/rgb/image_rect_color", Image, timeout=1.0)
        except: 
            image_msgs = rospy.wait_for_message("/camera/rgb/image_raw", Image)
        try:
            image_depth_msgs = rospy.wait_for_message("/camera/depth_registered/image_rect", Image, timeout=1.0)
        except:
            image_depth_msgs = rospy.wait_for_message("camera/depth_registered/image_raw", Image)
        ...
        if (self.client.get_state() == actionlib.GoalStatus.SUCCEEDED):
            ...
            for roi in rois:
                ...
                #scaled_image = cropped_image / 255.0
                
                cropped_depth_image = image_depth[top:bottom, left:right]
                #resized_depth_image = cv2.resize(cropped_depth_image, (28, 28))
                
                roi_data.append((cropped_image, cropped_depth_image, roi.x, roi.y, roi.z, roi.z_min, roi.z_max))            
