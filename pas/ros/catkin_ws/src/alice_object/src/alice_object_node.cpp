...
void PassThroughFilter(Pointcloud &point_cloud) {
	...
	passthrough_filter.setFilterLimits(-0.1, 0.8);
	...
	passthrough_filter.setFilterLimits(-0.4, 0.3);
	passthrough_filter.filter(point_cloud);
}
...
int main(int argc, char **argv) {
  ...
  ros::param::param(std::string("~camera_topic"), camera_topic, std::string("/camera/depth_registered/points"));
  ros::param::param(std::string("~base_link"), base_link, std::string("m1n6s200_link_base"));
  ros::param::param(std::string("~surface_distance_threshold"), surface_distance_threshold, float(0.01));
  ros::param::param(std::string("~cluster_tolerance"), cluster_tolerance, float(0.015));
  ros::param::param(std::string("~min_cluster_size"), min_cluster_size, int(500));
